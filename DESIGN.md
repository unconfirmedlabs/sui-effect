...
`Signed { digest, signed, signedAt }`, `Executed { digest, checkpoint?, at }`, `Failed { digest, reason, at }`, `NotApplied { digest, evidence, at }`, `Unknown { digest, signed, lastError, attempts, at }`, where `signed` is a `SignedTransaction { digest, bytes, signatures, sender, expiration? }` — one representation of signed bytes, shared with `SubmissionUnknown`, rather than the same five fields spelled out twice. `lastError` is the `SuiError.describe` line, so an entry is JSON with no error schema nested inside it. `checkpoint` and `SignedTransaction`'s `expiration`/`chain` are `Schema.optionalKey` (§10), so an entry either has the key with a value or does not have it — which is what JSON already gives a durable journal, so a line written before that rule decodes unchanged. `Signed` and `Unknown` are the unresolved tags; `Executed`, `Failed` and `NotApplied` are terminal. `NotApplied` is terminal and therefore a variant of its own: recording a proven-dead submission as `Unknown` would leave it in the durable journal's unresolved index forever, and `onUnresolved: "fail"` would refuse to build for the life of the store. This is the only place the lifecycle appears as a union; the program abstraction is the functions in section 6.

## 10. Errors

All `Schema.TaggedError` so they serialize. Flat tags, no inheritance.

**Every `?` field below is `Schema.optionalKey`, not `Schema.optional`**: on a shape this library constructs itself the key is either absent or carries a value its schema accepts, never present and set to `undefined` — the rule the Effect docs' "optional and default keys" page states, and the only one that reads as `status?: string` rather than `status?: string | undefined` under `exactOptionalPropertyTypes`. `Schema.optional` is kept exactly for the fields that mirror the SDK (`DynamicFieldEntry.childId`, `MoveLocation`, `CleverError`, `Event.json`, every `ExecutionReason` payload), where an object built by `@mysten/sui` really can carry a key with no value. Producers spread conditionally (`TransportError.fromUnknown`, `Executed.refOf`) rather than assigning `undefined`.
